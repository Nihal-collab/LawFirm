...
export default function BookConsultationSuccess() {
  ...
  if (loading) {
    return (
      <div className="min-h-[85vh] flex flex-col items-center justify-center bg-[#F8F5F0] dark:bg-[#121110] px-6 font-sans">
        <div className="max-w-md w-full card-premium text-center space-y-6">
          <div className="flex justify-center items-center">
            <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-[#8B6B57]"></div>
          </div>
          <h2 className="text-2xl font-serif font-medium text-[#171717] dark:text-[#F8F5F0]">Verifying Payment</h2>
    ...
    return (
      <div className="min-h-[85vh] flex items-center justify-center bg-[#F8F5F0] dark:bg-[#121110] px-6 font-sans">
        <div className="max-w-md w-full card-premium text-center space-y-6">
          ...
          <h2 className="text-3xl font-serif font-medium text-[#171717] dark:text-[#F8F5F0]">Verification Failed</h2>
          <p className="text-sm text-rose-600 dark:text-rose-450 leading-relaxed font-medium">
            {error}
          </p>
          <p className="text-xs text-[#6D6258] dark:text-[#C9C1B5] leading-relaxed font-light">
  ...
  return (
    <div className="min-h-[85vh] flex items-center justify-center bg-[#F8F5F0] dark:bg-[#121110] px-6 font-sans py-12">
      <div className="max-w-md w-full card-premium text-center space-y-6">
        ...
        <h2 className="text-3xl font-serif font-medium text-[#171717] dark:text-[#F8F5F0]">Booking Confirmed</h2>
        <p className="text-sm text-[#6D6258] dark:text-[#C9C1B5] leading-relaxed font-light">
        ...
        <div className="bg-[#F8F5F0] dark:bg-[#1C1A19] p-5 rounded-[12px] text-left text-xs space-y-3 border border-[#DDD5C8] dark:border-slate-800/80 text-[#6D6258] dark:text-[#C9C1B5]">
          <h3 className="font-semibold text-slate-800 dark:text-white uppercase tracking-wider text-[10px] pb-1 border-b border-[#DDD5C8]/50 dark:border-slate-800">Booking Coordinates</h3>
          <div><strong className="font-medium text-[#171717] dark:text-white">Practice Area:</strong> {booking?.service}</div>
          <div><strong className="font-medium text-[#171717] dark:text-white">Confirmed Date:</strong> {booking?.date}</div>
          <div><strong className="font-medium text-[#171717] dark:text-white">Confirmed Time:</strong> {booking?.time}</div>
        </div>

        {/* Payment Details */}
        <div className="bg-[#F8F5F0] dark:bg-[#1C1A19] p-5 rounded-[12px] text-left text-xs space-y-3 border border-[#DDD5C8] dark:border-slate-800/80 text-[#6D6258] dark:text-[#C9C1B5]">
          <h3 className="font-semibold text-slate-800 dark:text-white uppercase tracking-wider text-[10px] pb-1 border-b border-[#DDD5C8]/50 dark:border-slate-800">Payment Invoice</h3>
          <div><strong className="font-medium text-[#171717] dark:text-white">Payment Status:</strong> Successful</div>
          <div><strong className="font-medium text-[#171717] dark:text-white">Payment Method:</strong> PayPal</div>
          <div><strong className="font-medium text-[#171717] dark:text-white">Transaction ID:</strong> {booking?.paypalTransactionId}</div>
          <div><strong className="font-medium text-[#171717] dark:text-white">Order ID:</strong> {booking?.paypalOrderId}</div>
          <div><strong className="font-medium text-[#171717] dark:text-white">Amount Paid:</strong> {currencySymbol}{booking?.paymentAmount} {booking?.paymentCurrency}</div>
          <div><strong className="font-medium text-[#171717] dark:text-white">Payer Email:</strong> {booking?.payerEmail}</div>
        </div>

        <p className="text-[10px] text-[#6D6258]/60 text-center font-light pt-2">
